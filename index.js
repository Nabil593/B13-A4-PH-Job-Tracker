...
function sideCounter() {
  ...
  if (currentStatus === "all-btn") {
    counter.innerText = `${currentTotal} Jobs`;
    if (currentTotal === 0) {
    ...
    counter.innerText = `${interviewData.length} of ${currentTotal} Jobs`;
  } else if (currentStatus === "rejected-btn") {
    counter.innerText = `${rejectedData.length} of ${currentTotal} Jobs`;
  }
}
...
mainContainer.addEventListener("click", function (e) {
  if (e.target.classList.contains("interv-btn")) {
    ...
    const jobDiscription =
      mainParentNode.querySelector(".discription").innerText;

    let jobStatus = mainParentNode.querySelector(".status");
    jobStatus.innerText = "INTERVIEW";
    jobStatus.className =
      "w-30 text-center py-1.5 px-3 rounded-md bg-green-200 text-[14px] font-medium border-1 border-[#10B981] text-[#027a24]";
    ...
    const jobExist = interviewData.find(
    ...
    rejectedData = rejectedData.filter(
      (item) => item.jobName !== jobInfo.jobName,
    );
    switchButtons();
    ...
    jobStatus.className =
      "w-30 text-center py-1.5 px-3 rounded-md bg-red-200 text-[14px] font-medium border-1 border-red-700 text-red-600";
    ...
    const jobExist = rejectedData.find(
      (item) => item.jobName == jobInfo.jobName,
    );
    if (!jobExist) {
    ...
    interviewData = interviewData.filter(
      (item) => item.jobName !== jobInfo.jobName,
    );
    switchButtons();
    ...
    const jobCard = e.target.parentNode.parentNode;

    const jobName = jobCard.querySelector(".job-name").innerText;

    interviewData = interviewData.filter((item) => item.jobName !== jobName);

    rejectedData = rejectedData.filter((item) => item.jobName !== jobName);

    for (let job of JobDetails.children) {
      const name = job.querySelector(".job-name").innerText;
    ...
    total.innerText = JobDetails.children.length;
